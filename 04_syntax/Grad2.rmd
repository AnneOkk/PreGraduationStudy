...
packages <- c("mediation", "foreign", "tidyverse","stargazer","multicon", "ggplot2", "plyr", "reshape2", "readxl", 
              ...
              "countrycode", "WDI", "reticulate", "dplyr")
ipak(packages)
```
...
## Anonymize and save raw data

```{r}
library(haven)
df_sel$P_ID <- sample(length(df_sel$P_ID))
haven::write_sav(df, "../03_raw_and_processed_data/raw/df_raw.sav")
```
...
```{r}
df_sel <- read_sav("../03_raw_and_processed_data/raw/df_raw.sav")
```
...
mahad1 <- mahad(mahad1, plot = F, flag = T, confidence = 0.999, na.rm = TRUE)
...
mahad2 <- mahad(mahad2, plot = F, flag = T, confidence = 0.999, na.rm = TRUE)
...
mahad3 <- mahad(mahad3, plot = F, flag = T, confidence = 0.999, na.rm = TRUE)
...
mod.four_fac_orig <- '
CCONF =~ t1cconf_1 + t1cconf_2 + t1cconf_3 + t1cconf_4
CPL =~ t1cplan_1 + t1cplan_2 + t1cplan_3 + t1cplan_4 + t1cplan_5 + t1cplan_6 
WOR =~ t1wor_1 + t1wor_2 + t1wor_3
'
fit.mod.four_fac_origc<- cfa(mod.four_fac_orig, data = df, meanstructure = TRUE, std.lv = TRUE, estimator = 'MLR') 

summary(fit.mod.four_fac_origc, fit.measures=TRUE)
modindices(fit.mod.four_fac_origc, sort = TRUE, maximum.number = 10)
```
...
fit.mod.four_fac<- cfa(mod.four_fac, data = df, meanstructure = TRUE, std.lv = TRUE, estimator = 'MLR') 

summary(fit.mod.four_fac, fit.measures=TRUE)
...
fit <- sem(direct_effects, data=df, estimator='mlr',missing='fiml', fixed.x = F) #If fixed.x=T TRUE, the exogenous `x' covariates are considered fixed variables and the means, variances and covariances of these variables are fixed to their sample values. If FALSE, they are considered random
summary(fit, fit.measures=TRUE, standardized=TRUE, rsquare=TRUE)
...
fit_stab <- sem(direct_effects, data=df, estimator='mlr',missing='fiml', fixed.x = F) #If fixed.x=T TRUE, the exogenous `x' covariates are considered fixed variables and the means, variances and covariances of these variables are fixed to their sample values. If FALSE, they are considered random
...
fit_theor <- sem(direct_effects, data=df, estimator='mlr',missing='fiml', fixed.x = F) #If fixed.x=T TRUE, the exogenous `x' covariates are considered fixed variables and the means, variances and covariances of these variables are fixed to their sample values. If FALSE, they are considered random
...
fit_reci <- sem(direct_effects, data=df_sel, estimator='mlr',missing='fiml', fixed.x = F) #If fixed.x=T TRUE, the exogenous `x' covariates are considered fixed variables and the means, variances and covariances of these variables are fixed to their sample values. If FALSE, they are considered random
...
### Career confidence, controlling for T1 Worry 
